...
if __name__ == "__main__":
    ...
    cv2.imshow("mask", ovlay)
    cv2.imshow("PartitionOverlayed", result)
    cv2.waitKey(0)
    cv2.destroyAllWindows()

    ovlay = ImageResize(ovlay, 1.0/ratio)
    result = ImageResize(result,1.0/ratio)
    image = ImageResize(image, 1.0/ratio)

    cv2.imwrite(result_path + "Mask.jpeg", ovlay)
    cv2.imwrite(result_path + "PartitionOverlayed.jpeg", result)
    cv2.imwrite(result_path + "SeedsOverlayed.jpeg", image)
