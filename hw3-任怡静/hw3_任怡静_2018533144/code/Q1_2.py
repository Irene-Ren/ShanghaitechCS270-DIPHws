...
if __name__ == "__main__":
    ...
    image = ImageResize(image, 1.0/ratio)
    cv2.imwrite(result_path+"Multi_SeedsOverlayed.jpeg", image)
    mask = ImageResize(mask, 1.0/ratio)
    cv2.imshow("Segmented masks", np.uint8(mask))
    cv2.imwrite(result_path+"Multi_Masks.jpg", np.uint8(mask))
    result = cv2.addWeighted(np.uint8(showboard), 0.5, np.uint8(ovlay), 0.5, 0.1)
    result = ImageResize(result, 1.0 / ratio)
    cv2.imshow("Overlayed result", np.uint8(result))
    cv2.imwrite(result_path+"Multi_PartitionsOverlayed.jpg", np.uint8(result))
    cv2.waitKey(0)
